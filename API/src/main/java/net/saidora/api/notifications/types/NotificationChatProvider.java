...
public class NotificationChatProvider implements NotificationProvider {

    private final NotificationBuilder notificationBuilder;

    private String context = "";
    private String splitter = "";
    private final Set<TagResolver> resolvers = new HashSet<>(List.of(StandardTags.color()));

    public NotificationChatProvider(NotificationBuilder notificationBuilder) {
        this.notificationBuilder = notificationBuilder;
    }

    @Override
    public void send(CommandSender sender) {
        sender.sendMessage(getMessageBuilder().deserialize(ColorEnum.translateAlternateColorCodes(splitter.isEmpty() ? context : context.replace(splitter, "\n")), resolvers.toArray(new TagResolver[]{})));
    }
