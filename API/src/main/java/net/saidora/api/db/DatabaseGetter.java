...
public class DatabaseGetter {
    ...
    public void createOrUpdate(String name, Map<String, Object> values) {
        ...
        for (Object value : values.values()) {
            ObjectParser.getParser(value).ifPresentOrElse(objectObjectParser -> {
                try {
                    statement.setString(Math.min(id[0]++, values.size()), objectObjectParser.serialize(objectObjectParser.getValue()));
                ...
                try {
                    if(value instanceof Collection) {
                        statement.setString(id[0]++, String.join("..//", (Collection) value));
                    }
                    else {
                        statement.setObject(Math.min(id[0]++, values.size()), value);
                    };
                } catch (SQLException e) {
                    e.printStackTrace();
                }
