...
public class EventBuilder<T> {
    ...
    public EventBuilder(Class<T> clazz, Consumer<T> consumer) {
        ...
        if(consumers.isEmpty()) API.getInstance().getLogger().warning("Cannot find any event ('" + clazz.getName() + "')");
    }
    ...
    public void add(Consumer<T> consumer){
        ...
        if(eventLimiter.size() == -1){
            ...
            consumers.add(consumer);
        } else API.getInstance().getLogger().warning(eventLimiter.limitMessage().trim().isEmpty() ? "Amount of events \"" + clazz.getPackage() + "\" is too big. Cannot add new event." : eventLimiter.limitMessage());
    }
